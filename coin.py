...
not_supported_text = 'That currency is not supported. Supported currencies: USD, AUD, BRL, CAD, CHF, CNY, EUR, GBP, HKD, IDR, INR, JPY, KRW, MXN, RUB, TRY, BTC, ETH, XRP, LTC, ETC, DASH, XEM, MIOTA, XMR, BTS, STRAT, EOS, ZEC, BCC, WAVES'
...
async def on_message(message):
	...
	if message.content.lower().startswith('!btc') or message.content.lower().startswith('!bitcoin'):
		...
		await client.send_message(message.channel, 'Using the CoinMarketCap API.')
		
	elif message.content.lower().startswith('!dev'):
	
		await client.send_message(message.channel, 'Developed by Andy - #3557')
